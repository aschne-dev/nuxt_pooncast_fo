<template>
  <div class="flex flex-col items-center justify-between h-full px-5 py-5 rounded-xl shadow-2xl" aria-labelledby="blog-title-{{ blog.id }}">
    
    <!-- Titre de l'article et image -->
    <NuxtLink :to="`/poonblog/${generateSlug(blog.title)}/${blog.id}`" aria-labelledby="blog-title-{{ blog.id }}">
      <div class="flex flex-col items-center relative">
        <img :src="blog.visuel" :alt="'Image de l’article : ' + blog.title" class="rounded-2xl w-60 h-auto" />
        <h3 :id="'blog-title-' + blog.id" class="text-base font-syne xl:text-lg font-semibold text-center lg:text-start mt-5 w-60">
          {{ blog.title }}
        </h3>               
      </div>
    </NuxtLink>
    ...
    <div class="flex items-center justify-between px-5 lg:px-0 w-60 mt-5">
      <!-- Bouton Lire avec aria-label -->
      <NuxtLink :to="`/poonblog/${generateSlug(blog.title)}/${blog.id}`" class="mt-5 group" aria-label="'Lire l'article : ' + blog.title">
        <button class="btn">
          <span>Lire</span>
      ...
      <button @click="handleShare(blog.id, blog.title)" aria-label="'Partager l’article : ' + blog.title">
